...
pub fn run() -> Result<(), Error> {
    ...
    for num in numbers {
        data = data.iter().map(|arr| mark_item(arr, num)).collect();
        let winning_array = data.iter().find(|array| is_winning_board(array));
        if winning_array.is_some() {
            let sum: i32 = winning_array.unwrap().as_row_major().iter().filter(|&val| val != &MARKED).sum();
            let score: i64 = (sum * num) as i64;
            println!("Winning board: {:?}, \nsum {}, \nscore {}", winning_array.unwrap().as_rows(), sum, score);
            break;
        }
    }
